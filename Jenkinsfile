    ...
    stages {
        ...
        stage('Test Product Service') {
            steps {
                ...
                sh "mvn test -pl product -am -Drevision=${REVISION}"
            }
        }
        ...
        stage('Coverage Check') {
            steps {
                script {
                    ...
                    sh "mvn jacoco:report -pl product -Drevision=${REVISION}"
                    
                    // Lọc con số % từ file HTML
                    def coverageStr = sh(
                        script: "cat product/target/site/jacoco/index.html | grep -o 'Total[^%]*%' | grep -oE '[0-9]+%' | tr -d '%' | head -n 1",
                        returnStdout: true
                    ).trim()
